...
export const Connect = () => {
  ...
  useEffect(() => {
    ...
    const child = parent?.children.find((child) => child.key === queryUsing) ?? parent?.children?.[0]
    setSelectedChild(child?.key ?? '')
    if (child?.key) setQueryUsing(child.key)
    else setQueryUsing(null)

    const grandchild = child?.children.find((child) => child.key === queryWith) ?? child?.children?.[0]
